    ...
    // $response 	= $this->recaptcha->verifyResponse($recaptcha);
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			else
			{
				...
				{
					//set session nama
					// $nama	= $this->ion_auth->select_nama();
					// print_r($nama->name);
					// $this->session->set_userdata('nama', $nama->name);

					//set message dan redirect ke home apabila berhasil login

					if ($this->session->userdata('usertype') == '5') {
						$this->session->set_flashdata('message', '<div class="col-lg-12"><div class="alert alert-block alert-success"><i class="ace-icon fa fa-bullhorn green"></i> Selamat Anda Berhasil Login sebagai Customer </div></div>');
						...
						$this->data['message'] = "<div class='col-lg-12'><div class='alert alert-block alert-warning'><i class='ace-icon fa fa-bullhorn green'></i> Anda tidak punya akses sebagai customer</div></div>";
						$this->_render_page('front/auth/login', $this->data);
					}
